...
class Mario(pg.sprite.Sprite):
    def __init__(self):
        ...
        # FORCE Mario to be visible from the start - CRITICAL FIX
    ...
    def setup_state_booleans(self):
        ...
        self.test_invincible = True  # Modo de invencibilidade para teste - ATIVADO por padrão
    ...
    def get_image(self, x, y, width, height):
        ...
        # Use SRCALPHA for proper transparency handling
        image = pg.Surface([width, height], pg.SRCALPHA)
        rect = image.get_rect()

        image.blit(self.sprite_sheet, (0, 0), (x, y, width, height))
        # REMOVED colorkey completely - this was causing Mario to be invisible
        # image.set_colorkey(c.BLACK)  # This was the problem!
        image = pg.transform.scale(image,
                                   (int(rect.width*c.SIZE_MULTIPLIER),
                                    int(rect.height*c.SIZE_MULTIPLIER)))
        
        # Convert with alpha for proper transparency
        image = image.convert_alpha()
        
        # FORCE all images to be fully visible - CRITICAL FIX
        image.set_alpha(255)
    ...
    def update(self, keys, game_info, fire_group):
        ...
        # Toggle test invincibility with 'I' key
        if keys[pg.K_i] and not hasattr(self, 'i_key_pressed'):
            self.test_invincible = not self.test_invincible
            self.i_key_pressed = True
            # Play sound feedback when toggling invincibility
            if hasattr(setup, 'SFX') and 'powerup' in setup.SFX:
                setup.SFX['powerup'].play()
            print(f"Test Invincibility: {'ON' if self.test_invincible else 'OFF'}")
        elif not keys[pg.K_i]:
            self.i_key_pressed = False
            
        self.handle_state(keys, fire_group)
        self.check_for_special_state()
        self.animation()
        
        # FINAL AND ABSOLUTE CHECK - Mario MUST be visible - NO EXCEPTIONS
        if hasattr(self, 'image') and hasattr(self.image, 'set_alpha'):
            self.image.set_alpha(255)  # ALWAYS visible, regardless of any other condition
            
        # Additional safety check for test invincible mode
        if self.test_invincible and hasattr(self, 'image') and hasattr(self.image, 'set_alpha'):
            self.image.set_alpha(255)
    ...
    def check_for_special_state(self):
        ...
        self.check_if_fire()
        # Removed check_if_hurt_invincible() to prevent invisibility issues
        self.check_if_crouching()


    def check_if_invincible(self):
        # Don't interfere with frames if in test invincible mode
        if self.test_invincible:
            return
    ...
    def check_if_hurt_invincible(self):
        """Check if Mario is still temporarily invincible after getting hurt - DISABLED"""
        # This method is completely disabled to prevent invisibility issues
        # Mario will remain visible and invincible when test_invincible is True
        self.hurt_invincible = False
        self.hurt_invisible_timer = 0
        self.hurt_invisible_timer2 = 0
    ...
    def hurt_invincible_check(self):
        """Makes Mario invincible on a fixed interval - DISABLED to prevent invisibility"""
        # This method is completely disabled to prevent Mario from becoming invisible
        # Mario will remain visible at all times
        pass
    ...
    def animation(self):
        ...
        # ALWAYS ensure Mario is fully visible - no exceptions
        if hasattr(self.image, 'set_alpha'):
            self.image.set_alpha(255)
